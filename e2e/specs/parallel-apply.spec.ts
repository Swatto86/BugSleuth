...
describe("parallel repository fixing", () => {
  ...
  it("runs Claude and OpenCode independently and retains each model and result", async function () {
    if (live) this.skip();
    const root = process.env["BUGSLEUTH_E2E_ROOT"]!;
    const repos = ["fix-claude", "fix-opencode"].map((name) =>
      path.join(root, name),
    );
    ...
    for (const [index, vendor] of ["claude", "opencode"].entries()) {
      await $("#repository-result").selectByAttribute(
        "value",
        String(index + 1),
      );
      await $("#apply-vendor").selectByAttribute("value", vendor);
      await $("#apply-model input").setValue(
        index === 0 ? "haiku" : "local-fixture/fixer:latest",
      );
    ...
    assert.equal(await $("#apply-vendor").getValue(), "claude");
    assert.equal(await $("#apply-model input").getValue(), "haiku");
    await expect($("#apply-fixes")).toBeDisabled();
    await expect($("#run")).toBeDisabled();
    await expect($("#apply-jobs")).toHaveText(
      /fix-claude.*claude:haiku[\s\S]*fix-opencode.*opencode:local-fixture\/fixer:latest/,
    );
    await browser.waitUntil(
      () =>
        fs
    ...
    await expect($("#output")).toHaveText(/Fixed the bulk discount threshold/);
    await expect($("#run")).toBeDisabled();
    ...
    for (const [index, repo] of repos.entries()) {
      ...
      assert.equal(
        await $("#apply-vendor").getValue(),
        index ? "opencode" : "claude",
      );
    }
    ...
    await browser.waitUntil(
      () =>
        JSON.parse(fs.readFileSync(settingsFile, "utf8")).apply_repositories?.[
          repos[1]
        ]?.apply_model === "opencode:local-fixture/fixer:latest",
      { timeout: 10_000 },
    );
  });
});
