...
pub async fn apply(clusters: &mut [Cluster], request: Request<'_>) -> Outcome {
    ...
    let _slot = match crate::vendor_slots::acquire(crate::sweep::Vendor::Claude).await {
        Ok(slot) => slot,
        Err(error) => {
            return Outcome {
                note: format!("{UNGRADED}: no Claude session could be reserved ({error})"),
                ..Outcome::default()
            };
        }
    };
...
mod evidence;
use evidence::{acknowledgement, commentary_at};
