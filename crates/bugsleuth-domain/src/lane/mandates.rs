//! What each lane is told to look for.
//!
//! Split from `lane.rs` at the hard line cap, along the seam already there: that
//! file answers "what is a lane", and this one is the prose a sweep is actually
//! handed. Every mandate here was written against a *measured* miss, and the
//! tests below record which one — so an edit that drops a section has to argue
//! with the evidence that put it there.
//!
//! Each states what is in scope *and* what is out. The out-of-scope half matters
//! as much: the failure mode lanes exist to prevent is all of them drifting back
//! to the same generic findings.

pub(super) const CORRECTNESS_MANDATE: &str = "\
...
pub(super) const SECURITY_MANDATE: &str = "\
...
pub(super) const CONTRACT_MANDATE: &str = "\
...
pub(super) const UX_MANDATE: &str = "\
...
pub(super) const GATE_MANDATE: &str = "Review the CHECKS, not the code they are meant to protect. Your subject is the test suite, the CI workflows and the build or verification scripts. Every other reviewer is reading `src`; you are reading the thing that is supposed to catch them when they are wrong.

The defect you are hunting has one shape: **a check that passes whether or not the behaviour it names is correct.** A green run is what everyone trusts, so a check that cannot fail is worse than no check at all — it occupies the place where a real one would go, and it reads as protection to everyone who sees it.

# What to look for

**An assertion that cannot fail.** A comparison that is true for every value the type can hold (`assert!(x >= 0)` on an unsigned integer, `assert!(v.len() >= 0)`), a check of a literal against itself, an assertion on a value the test computed rather than on the code's output.

**A scan that can match nothing.** Any check that greps source text, walks files, splits on a separator, parses output, or filters a list, and then asserts the result is empty or below a limit. An empty set satisfies every such assertion. Ask what the scan matches when the pattern is wrong — if the answer is 'nothing, and it passes', that is the defect. Reading the pattern against the file it scans is the whole job: a separator with the wrong indentation, a regex anchored to the wrong line, a directory walk that never recurses.

**A result that is never consumed.** A fallible call whose `Result`, error code, or exit status is discarded — `let _ = ...`, `|| true`, a pipeline whose last stage is `tail`, `head` or `tee` so the shell reports that stage's status and not the command's, a loop that records failures into a variable nothing reads afterwards.

**A test that has been switched off.** `#[ignore]`, `skip`, `xfail`, `it.skip`, a commented-out assertion, a filter that excludes a named case. Check the stated reason against the code: 'flaky' attached to something that fails deterministically is a known defect being hidden, not a flake.

**A check with nothing left to guard.** An assertion about a function, flag or file that no longer exists in the form it names, and a test whose subject was deleted or rewritten around it.

**A gate that is not wired to anything.** A script no workflow calls, a hook that is never installed, a job whose failure does not fail the build.

# How to report

Say what change to the production code SHOULD make the check fail, and why it does not. If you cannot name that change, you have not found this defect and should not report it.

OUT OF SCOPE for this lane: defects in the production code itself — other reviewers cover those, and reporting them here wastes the one pass that reads the tests. Missing coverage is out of scope too: 'there is no test for X' is an observation anyone can make about any codebase. A check that exists and does nothing is the finding.";
...
mod tests {
    ...
    fn the_gate_mandate_names_the_shapes_it_was_graded_on() {
        ...
        // lanes cover. The miss was G1 both times it happened, so a single pass
        // of this lane is worth about three and a half of four: recall here
        // argues for `passes: 2` rather than for more mandate.
        //
        // Then against this repository, where it found two real ones nobody had
        // noticed: a round-trip test whose `unwrap_or` fallback was the value
        // under test, and a `tsconfig.json` that excluded every `*.test.ts`
        // from the only type-checking the gate does.
        let mandate = Lane::Gate.mandate();
        assert!(
            mandate.contains("cannot fail"),
            "nothing states the shape of the defect: a check that passes either way"
        );
        assert!(
            mandate.contains("match nothing"),
            "nothing asks about a scan that returns an empty set and passes"
        );
        assert!(
            mandate.contains("never consumed"),
            "nothing asks about a discarded exit status or Result"
        );
        assert!(
            mandate.contains("switched off"),
            "nothing asks about ignored or skipped tests"
        );
        // The out-of-scope half is what stops it drifting into the other four
        // lanes and reporting the production defects they already cover.
        assert!(
            mandate.contains("Missing coverage is out of scope"),
            "nothing rules out 'there is no test for X', which is true of everything"
        );
    }
}
